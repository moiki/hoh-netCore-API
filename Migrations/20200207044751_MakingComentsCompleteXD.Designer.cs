// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using hohsys.API.data;

namespace hohsys.API.Migrations
{
    [DbContext(typeof(HoHDbContext))]
    [Migration("20200207044751_MakingComentsCompleteXD")]
    partial class MakingComentsCompleteXD
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn)
                .HasAnnotation("ProductVersion", "3.1.1")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<int>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<string>("ClaimType")
                        .HasColumnType("text");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("text");

                    b.Property<int>("RoleId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<int>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<string>("ClaimType")
                        .HasColumnType("text");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("text");

                    b.Property<int>("UserId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<int>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasColumnType("text");

                    b.Property<string>("ProviderKey")
                        .HasColumnType("text");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("text");

                    b.Property<int>("UserId")
                        .HasColumnType("integer");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<int>", b =>
                {
                    b.Property<int>("UserId")
                        .HasColumnType("integer");

                    b.Property<string>("LoginProvider")
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .HasColumnType("text");

                    b.Property<string>("Value")
                        .HasColumnType("text");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens");
                });

            modelBuilder.Entity("hohsys.API.models.Clinic", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasComment("Llave primaria de la entidad")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<string>("Address")
                        .HasColumnType("character varying(500)")
                        .HasComment("Propiedad de borrado lógico")
                        .HasMaxLength(500);

                    b.Property<string>("City")
                        .HasColumnType("character varying(100)")
                        .HasComment("Ciudad donde se ubica la clínica")
                        .HasMaxLength(100);

                    b.Property<DateTimeOffset>("DateAdded")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha en la que se agregó el registro");

                    b.Property<DateTimeOffset>("DateUpdated")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha de última actualización del registro");

                    b.Property<string>("Description")
                        .HasColumnType("character varying(500)")
                        .HasComment("Descripción general de la clínica")
                        .HasMaxLength(500);

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasComment("Propiedad de borrado lógico");

                    b.Property<string>("Name")
                        .HasColumnType("character varying(60)")
                        .HasComment("Nombre de la clinica en el registro")
                        .HasMaxLength(60);

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("character varying(100)")
                        .HasComment("Número de teléfono de la clínica")
                        .HasMaxLength(100);

                    b.Property<string>("UserAdded")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que creó el registro");

                    b.Property<string>("UserUpdated")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que realizó la última actualización");

                    b.HasKey("Id");

                    b.ToTable("Clinics");
                });

            modelBuilder.Entity("hohsys.API.models.Destination", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasComment("Llave primaria de la entidad")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<DateTimeOffset>("DateAdded")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha en la que se agregó el registro");

                    b.Property<DateTimeOffset>("DateUpdated")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha de última actualización del registro");

                    b.Property<string>("Description")
                        .HasColumnType("text")
                        .HasComment("Descrición general del destino");

                    b.Property<string>("DestinationName")
                        .HasColumnType("text")
                        .HasComment("Titulo o nombre del destino");

                    b.Property<string>("GoogleMapEmbedUrl")
                        .HasColumnType("text")
                        .HasComment("Url de google Maps de la localización geográfica del destino.");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasComment("Propiedad de borrado lógico");

                    b.Property<string>("Picture")
                        .HasColumnType("text")
                        .HasComment("Imagen principal del registro");

                    b.Property<string>("UserAdded")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que creó el registro");

                    b.Property<string>("UserUpdated")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que realizó la última actualización");

                    b.Property<int?>("WorkRouteId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("WorkRouteId");

                    b.ToTable("Destinations");
                });

            modelBuilder.Entity("hohsys.API.models.Email", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasComment("Llave primaria de la entidad.")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<DateTimeOffset>("DateAdded")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha en la que se agregó el registro.");

                    b.Property<DateTimeOffset>("DateUpdated")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha de última actualización del registro.");

                    b.Property<string>("EmailAddress")
                        .HasColumnType("text")
                        .HasComment("Corresponde a la dirección de correo electrónico.");

                    b.Property<int?>("EmployeeId")
                        .HasColumnType("integer");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasComment("Propiedad de borrado lógico.");

                    b.Property<string>("UserAdded")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que creó el registro.");

                    b.Property<string>("UserUpdated")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que realizó la última actualización.");

                    b.HasKey("Id");

                    b.HasIndex("EmployeeId");

                    b.ToTable("Emails");
                });

            modelBuilder.Entity("hohsys.API.models.Employee", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasComment("Llave primaria de la entidad.")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<string>("Address")
                        .HasColumnType("character varying(400)")
                        .HasComment("Dirección domiciliar del empleado.")
                        .HasMaxLength(400);

                    b.Property<int>("ClinicId")
                        .HasColumnType("integer");

                    b.Property<DateTimeOffset>("DateAdded")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha en la que se agregó el registro.");

                    b.Property<DateTimeOffset>("DateUpdated")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha de última actualización del registro.");

                    b.Property<string>("FirstName1")
                        .HasColumnType("character varying(100)")
                        .HasComment("Primer nombre del empleado.")
                        .HasMaxLength(100);

                    b.Property<string>("FirstName2")
                        .HasColumnType("character varying(100)")
                        .HasComment("Segundo nombre del empleado.")
                        .HasMaxLength(100);

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasComment("Propiedad de borrado lógico.");

                    b.Property<string>("LastName1")
                        .HasColumnType("character varying(100)")
                        .HasComment("Primer apellido del empleado.")
                        .HasMaxLength(100);

                    b.Property<string>("LastName2")
                        .HasColumnType("character varying(100)")
                        .HasComment("Segundo apellido del empleado.")
                        .HasMaxLength(100);

                    b.Property<string>("ProfilePicture")
                        .HasColumnType("text")
                        .HasComment("Foto de perdil de empleado.");

                    b.Property<string>("UserAdded")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que creó el registro.");

                    b.Property<string>("UserUpdated")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que realizó la última actualización.");

                    b.Property<int?>("WorkRouteId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("ClinicId");

                    b.HasIndex("WorkRouteId");

                    b.ToTable("Employees");
                });

            modelBuilder.Entity("hohsys.API.models.MediaCategory", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasComment("Llave primaria de la entidad.")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<string>("CategoryName")
                        .HasColumnType("text")
                        .HasComment("Nombre de la categoría.");

                    b.Property<DateTimeOffset>("DateAdded")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha en la que se agregó el registro.");

                    b.Property<DateTimeOffset>("DateUpdated")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha de última actualización del registro.");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasComment("Propiedad de borrado lógico.");

                    b.Property<string>("UserAdded")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que creó el registro.");

                    b.Property<string>("UserUpdated")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que realizó la última actualización.");

                    b.HasKey("Id");

                    b.ToTable("MediaCategories");
                });

            modelBuilder.Entity("hohsys.API.models.MedicalIndication", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasComment("Llave primaria de la entidad.")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<string>("Body")
                        .HasColumnType("text")
                        .HasComment("Cuerpo o descripción de registro de indicación.");

                    b.Property<DateTimeOffset>("DateAdded")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha en la que se agregó el registro.");

                    b.Property<DateTimeOffset>("DateUpdated")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha de última actualización del registro.");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasComment("Propiedad de borrado lógico.");

                    b.Property<int?>("MedicalRecordId")
                        .HasColumnType("integer");

                    b.Property<string>("UserAdded")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que creó el registro.");

                    b.Property<int?>("UserId")
                        .HasColumnType("integer");

                    b.Property<string>("UserUpdated")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que realizó la última actualización.");

                    b.HasKey("Id");

                    b.HasIndex("MedicalRecordId");

                    b.HasIndex("UserId");

                    b.ToTable("MedicalIndications");
                });

            modelBuilder.Entity("hohsys.API.models.MedicalRecord", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasComment("Llave primaria de la entidad")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<DateTimeOffset>("DateAdded")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha en la que se agregó el registro");

                    b.Property<DateTimeOffset>("DateUpdated")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha de última actualización del registro");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasComment("Propiedad de borrado lógico");

                    b.Property<int?>("PacientId")
                        .HasColumnType("integer");

                    b.Property<string>("UserAdded")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que creó el registro");

                    b.Property<string>("UserUpdated")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que realizó la última actualización");

                    b.HasKey("Id");

                    b.HasIndex("PacientId");

                    b.ToTable("MedicalRecords");
                });

            modelBuilder.Entity("hohsys.API.models.Pacient", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasComment("Llave primaria de la entidad")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<string>("Address")
                        .HasColumnType("character varying(400)")
                        .HasComment("Dirección domiciliar del paciente")
                        .HasMaxLength(400);

                    b.Property<DateTime>("Birth")
                        .HasColumnType("timestamp without time zone");

                    b.Property<int?>("ClinicId")
                        .HasColumnType("integer");

                    b.Property<DateTimeOffset>("DateAdded")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha en la que se agregó el registro");

                    b.Property<DateTimeOffset>("DateUpdated")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha de última actualización del registro");

                    b.Property<string>("FirstName1")
                        .HasColumnType("character varying(100)")
                        .HasComment("Primer nombre del paciente")
                        .HasMaxLength(100);

                    b.Property<string>("FirstName2")
                        .HasColumnType("character varying(100)")
                        .HasComment("Segundo nombre del paciente")
                        .HasMaxLength(100);

                    b.Property<bool>("Gender")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasComment("Propiedad de borrado lógico");

                    b.Property<string>("LastName1")
                        .HasColumnType("character varying(100)")
                        .HasComment("Primer apellido del paciente")
                        .HasMaxLength(100);

                    b.Property<string>("LastName2")
                        .HasColumnType("character varying(100)")
                        .HasComment("Segundo apellido del paciente")
                        .HasMaxLength(100);

                    b.Property<string>("UserAdded")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que creó el registro");

                    b.Property<string>("UserUpdated")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que realizó la última actualización");

                    b.Property<int?>("WorkRouteId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("ClinicId");

                    b.HasIndex("WorkRouteId");

                    b.ToTable("Pacients");
                });

            modelBuilder.Entity("hohsys.API.models.PacientComentary", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasComment("Llave primaria de la entidad.")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<string>("Body")
                        .HasColumnType("text")
                        .HasComment("Cuerpo del comentario.");

                    b.Property<DateTimeOffset>("DateAdded")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha en la que se agregó el registro.");

                    b.Property<DateTimeOffset>("DateUpdated")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha de última actualización del registro.");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasComment("Propiedad de borrado lógico.");

                    b.Property<int?>("PacientId")
                        .HasColumnType("integer");

                    b.Property<string>("UserAdded")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que creó el registro.");

                    b.Property<int?>("UserId")
                        .HasColumnType("integer");

                    b.Property<string>("UserUpdated")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que realizó la última actualización.");

                    b.HasKey("Id");

                    b.HasIndex("PacientId");

                    b.HasIndex("UserId");

                    b.ToTable("PatientComentaries");
                });

            modelBuilder.Entity("hohsys.API.models.PacientMedia", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasComment("Llave primaria de la entidad")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<DateTimeOffset>("DateAdded")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha en la que se agregó el registro");

                    b.Property<DateTimeOffset>("DateUpdated")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha de última actualización del registro");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasComment("Propiedad de borrado lógico");

                    b.Property<int?>("MediaCategoryId")
                        .HasColumnType("integer");

                    b.Property<int?>("PacientId")
                        .HasColumnType("integer");

                    b.Property<string>("Title")
                        .HasColumnType("text");

                    b.Property<string>("Url")
                        .HasColumnType("text")
                        .HasComment("dirección de ubicación del archivo al que refiere el registro");

                    b.Property<string>("UserAdded")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que creó el registro");

                    b.Property<string>("UserUpdated")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que realizó la última actualización");

                    b.HasKey("Id");

                    b.HasIndex("MediaCategoryId");

                    b.HasIndex("PacientId");

                    b.ToTable("PacientMedia");
                });

            modelBuilder.Entity("hohsys.API.models.PhoneNumber", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasComment("Llave primaria de la entidad.")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<DateTimeOffset>("DateAdded")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha en la que se agregó el registro.");

                    b.Property<DateTimeOffset>("DateUpdated")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha de última actualización del registro.");

                    b.Property<int?>("EmployeeId")
                        .HasColumnType("integer");

                    b.Property<string>("Extention")
                        .HasColumnType("text")
                        .HasComment("Número de extensión del número.");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasComment("Propiedad de borrado lógico.");

                    b.Property<string>("UserAdded")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que creó el registro.");

                    b.Property<string>("UserUpdated")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que realizó la última actualización.");

                    b.Property<string>("number")
                        .HasColumnType("text")
                        .HasComment("Corresponde al registro número de teléfono.");

                    b.HasKey("Id");

                    b.HasIndex("EmployeeId");

                    b.ToTable("PhoneNumbers");
                });

            modelBuilder.Entity("hohsys.API.models.Position", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasComment("Llave primaria de la entidad.")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<DateTimeOffset>("DateAdded")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha en la que se agregó el registro.");

                    b.Property<DateTimeOffset>("DateUpdated")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha de última actualización del registro.");

                    b.Property<string>("Description")
                        .HasColumnType("text")
                        .HasComment("Descripción del cargo.");

                    b.Property<int?>("EmployeeId")
                        .HasColumnType("integer");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasComment("Propiedad de borrado lógico.");

                    b.Property<string>("PositionName")
                        .HasColumnType("text")
                        .HasComment("Nombre del cargo o posición que se ejerce en una clínica.");

                    b.Property<string>("UserAdded")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que creó el registro.");

                    b.Property<string>("UserUpdated")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que realizó la última actualización.");

                    b.HasKey("Id");

                    b.HasIndex("EmployeeId");

                    b.ToTable("Positions");
                });

            modelBuilder.Entity("hohsys.API.models.Role", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .HasColumnType("character varying(256)")
                        .HasMaxLength(256);

                    b.Property<string>("NormalizedName")
                        .HasColumnType("character varying(256)")
                        .HasMaxLength(256);

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasName("RoleNameIndex");

                    b.ToTable("AspNetRoles");
                });

            modelBuilder.Entity("hohsys.API.models.Schedule", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasComment("Llave primaria de la entidad.")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<DateTimeOffset>("DateAdded")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha en la que se agregó el registro.");

                    b.Property<DateTimeOffset>("DateUpdated")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha de última actualización del registro.");

                    b.Property<DateTimeOffset>("Day")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Nombre del dia que corresponda al registro de horario creado.");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasComment("Propiedad de borrado lógico.");

                    b.Property<string>("State")
                        .HasColumnType("text")
                        .HasComment("Estado actual de un horario (Pendiente, ejecutado, cancelado).");

                    b.Property<string>("UserAdded")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que creó el registro.");

                    b.Property<string>("UserUpdated")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que realizó la última actualización.");

                    b.Property<int?>("WorkRouteId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("WorkRouteId");

                    b.ToTable("Schedules");
                });

            modelBuilder.Entity("hohsys.API.models.User", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasComment("Llave primaria de la entidad")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("integer");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("text");

                    b.Property<DateTimeOffset>("DateAdded")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha en la que se agregó el registro");

                    b.Property<DateTimeOffset>("DateUpdated")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha de última actualización del registro");

                    b.Property<string>("Email")
                        .HasColumnType("character varying(256)")
                        .HasMaxLength(256);

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasComment("Propiedad de borrado lógico");

                    b.Property<string>("Lastname")
                        .HasColumnType("text")
                        .HasComment("Apellido de un usuario. para este registro solo se almacena un apellido de usuario");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("boolean");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Name")
                        .HasColumnType("text")
                        .HasComment("Nombre de usuario. En este caso solo se guarda el primer nombre de un usuario.");

                    b.Property<string>("NormalizedEmail")
                        .HasColumnType("character varying(256)")
                        .HasMaxLength(256);

                    b.Property<string>("NormalizedUserName")
                        .HasColumnType("character varying(256)")
                        .HasMaxLength(256);

                    b.Property<string>("PasswordHash")
                        .HasColumnType("text");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("text");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("boolean");

                    b.Property<string>("Profession")
                        .HasColumnType("text");

                    b.Property<string>("ProfilePicture")
                        .HasColumnType("text");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("text");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("boolean");

                    b.Property<string>("UserName")
                        .HasColumnType("character varying(256)")
                        .HasMaxLength(256);

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .HasName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasName("UserNameIndex");

                    b.ToTable("AspNetUsers");
                });

            modelBuilder.Entity("hohsys.API.models.UserRole", b =>
                {
                    b.Property<int>("UserId")
                        .HasColumnType("integer");

                    b.Property<int>("RoleId")
                        .HasColumnType("integer");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles");
                });

            modelBuilder.Entity("hohsys.API.models.WorkRoute", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasComment("Llave primaria de la entidad.")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<int?>("ClinicId")
                        .HasColumnType("integer");

                    b.Property<DateTimeOffset>("DateAdded")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha en la que se agregó el registro.");

                    b.Property<DateTimeOffset>("DateUpdated")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha de última actualización del registro.");

                    b.Property<string>("Description")
                        .HasColumnType("character varying(500)")
                        .HasComment("Descripción general de la ruta de trabajo.")
                        .HasMaxLength(500);

                    b.Property<string>("FeaturedImage")
                        .HasColumnType("text")
                        .HasComment("Imagen titular del registro de ruta.");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasComment("Propiedad de borrado lógico.");

                    b.Property<string>("RouteName")
                        .HasColumnType("character varying(200)")
                        .HasComment("Título que corresponde al registro de rutas de trabajo.")
                        .HasMaxLength(200);

                    b.Property<string>("UserAdded")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que creó el registro.");

                    b.Property<string>("UserUpdated")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que realizó la última actualización.");

                    b.HasKey("Id");

                    b.HasIndex("ClinicId");

                    b.ToTable("WorkRoutes");
                });

            modelBuilder.Entity("hohsys.API.models.WorkRouteComentary", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasComment("Llave primaria de la entidad.")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<string>("Body")
                        .HasColumnType("text")
                        .HasComment("Cuerpo del comentario.");

                    b.Property<DateTimeOffset>("DateAdded")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha en la que se agregó el registro.");

                    b.Property<DateTimeOffset>("DateUpdated")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha de última actualización del registro.");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasComment("Propiedad de borrado lógico.");

                    b.Property<string>("UserAdded")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que creó el registro.");

                    b.Property<int?>("UserId")
                        .HasColumnType("integer");

                    b.Property<string>("UserUpdated")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que realizó la última actualización.");

                    b.Property<int?>("WorkRouteId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.HasIndex("WorkRouteId");

                    b.ToTable("WorkRouteComentaries");
                });

            modelBuilder.Entity("hohsys.API.models.WorkRouteMedia", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasComment("Llave primaria de la entidad")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<DateTimeOffset>("DateAdded")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha en la que se agregó el registro");

                    b.Property<DateTimeOffset>("DateUpdated")
                        .HasColumnType("timestamp with time zone")
                        .HasComment("Fecha de última actualización del registro");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasComment("Propiedad de borrado lógico");

                    b.Property<int?>("MediaCategoryId")
                        .HasColumnType("integer");

                    b.Property<string>("Title")
                        .HasColumnType("text");

                    b.Property<string>("Url")
                        .HasColumnType("text")
                        .HasComment("dirección de ubicación del archivo al que refiere el registro");

                    b.Property<string>("UserAdded")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que creó el registro");

                    b.Property<string>("UserUpdated")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasComment("Usuario que realizó la última actualización");

                    b.Property<int?>("WorkRouteId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("MediaCategoryId");

                    b.HasIndex("WorkRouteId");

                    b.ToTable("WorkRouteMedia");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<int>", b =>
                {
                    b.HasOne("hohsys.API.models.Role", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<int>", b =>
                {
                    b.HasOne("hohsys.API.models.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<int>", b =>
                {
                    b.HasOne("hohsys.API.models.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<int>", b =>
                {
                    b.HasOne("hohsys.API.models.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("hohsys.API.models.Destination", b =>
                {
                    b.HasOne("hohsys.API.models.WorkRoute", "WorkRoute")
                        .WithMany("Destinations")
                        .HasForeignKey("WorkRouteId");
                });

            modelBuilder.Entity("hohsys.API.models.Email", b =>
                {
                    b.HasOne("hohsys.API.models.Employee", "Employee")
                        .WithMany("Emails")
                        .HasForeignKey("EmployeeId");
                });

            modelBuilder.Entity("hohsys.API.models.Employee", b =>
                {
                    b.HasOne("hohsys.API.models.Clinic", "Clinic")
                        .WithMany("Employees")
                        .HasForeignKey("ClinicId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("hohsys.API.models.WorkRoute", "WorkRoute")
                        .WithMany("Employees")
                        .HasForeignKey("WorkRouteId");
                });

            modelBuilder.Entity("hohsys.API.models.MedicalIndication", b =>
                {
                    b.HasOne("hohsys.API.models.MedicalRecord", "MedicalRecord")
                        .WithMany("MedicalIndications")
                        .HasForeignKey("MedicalRecordId");

                    b.HasOne("hohsys.API.models.User", "User")
                        .WithMany("MedicalIndications")
                        .HasForeignKey("UserId");
                });

            modelBuilder.Entity("hohsys.API.models.MedicalRecord", b =>
                {
                    b.HasOne("hohsys.API.models.Pacient", "Pacient")
                        .WithMany("MedicalRecords")
                        .HasForeignKey("PacientId");
                });

            modelBuilder.Entity("hohsys.API.models.Pacient", b =>
                {
                    b.HasOne("hohsys.API.models.Clinic", "Clinic")
                        .WithMany("Pacients")
                        .HasForeignKey("ClinicId");

                    b.HasOne("hohsys.API.models.WorkRoute", "WorkRoute")
                        .WithMany("Pacients")
                        .HasForeignKey("WorkRouteId");
                });

            modelBuilder.Entity("hohsys.API.models.PacientComentary", b =>
                {
                    b.HasOne("hohsys.API.models.Pacient", "Pacient")
                        .WithMany("PacientComentaries")
                        .HasForeignKey("PacientId");

                    b.HasOne("hohsys.API.models.User", "User")
                        .WithMany("PacientComentaries")
                        .HasForeignKey("UserId");
                });

            modelBuilder.Entity("hohsys.API.models.PacientMedia", b =>
                {
                    b.HasOne("hohsys.API.models.MediaCategory", "MediaCategory")
                        .WithMany("PacientMedia")
                        .HasForeignKey("MediaCategoryId");

                    b.HasOne("hohsys.API.models.Pacient", "Pacient")
                        .WithMany("PacientMedia")
                        .HasForeignKey("PacientId");
                });

            modelBuilder.Entity("hohsys.API.models.PhoneNumber", b =>
                {
                    b.HasOne("hohsys.API.models.Employee", "Employee")
                        .WithMany("PhoneNumbers")
                        .HasForeignKey("EmployeeId");
                });

            modelBuilder.Entity("hohsys.API.models.Position", b =>
                {
                    b.HasOne("hohsys.API.models.Employee", "Employee")
                        .WithMany("Positions")
                        .HasForeignKey("EmployeeId");
                });

            modelBuilder.Entity("hohsys.API.models.Schedule", b =>
                {
                    b.HasOne("hohsys.API.models.WorkRoute", "WorkRoute")
                        .WithMany("Schedules")
                        .HasForeignKey("WorkRouteId");
                });

            modelBuilder.Entity("hohsys.API.models.UserRole", b =>
                {
                    b.HasOne("hohsys.API.models.Role", "Role")
                        .WithMany("UserRoles")
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("hohsys.API.models.User", "User")
                        .WithMany("UserRoles")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("hohsys.API.models.WorkRoute", b =>
                {
                    b.HasOne("hohsys.API.models.Clinic", "Clinic")
                        .WithMany()
                        .HasForeignKey("ClinicId");
                });

            modelBuilder.Entity("hohsys.API.models.WorkRouteComentary", b =>
                {
                    b.HasOne("hohsys.API.models.User", "User")
                        .WithMany("WorkRouteComentaries")
                        .HasForeignKey("UserId");

                    b.HasOne("hohsys.API.models.WorkRoute", "WorkRoute")
                        .WithMany("WorkRouteComentaries")
                        .HasForeignKey("WorkRouteId");
                });

            modelBuilder.Entity("hohsys.API.models.WorkRouteMedia", b =>
                {
                    b.HasOne("hohsys.API.models.MediaCategory", "MediaCategory")
                        .WithMany("WorkRouteMedia")
                        .HasForeignKey("MediaCategoryId");

                    b.HasOne("hohsys.API.models.WorkRoute", "WorkRoute")
                        .WithMany("WorkRouteMedia")
                        .HasForeignKey("WorkRouteId");
                });
#pragma warning restore 612, 618
        }
    }
}
